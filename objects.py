...
class Piece:
    ...
    def move(self, move):  # moves a piece: adds move to the moves dictionary, updates the board and itself
        ...
        if move.special == 'ep':  # if the move is an en passent
            ...
            newpos = (oldpos[0], rook_column[oldpos[1]])  # new column is 3 if left rook or 5 if left rook
...
class Game:
    ...
    def __getitem__(self, index):
        if type(index) == tuple:  # tuple
            return self.state[index[0]][index[1]]
        elif type(index) == int:  # int
    ...
    def move(self, move, selected=tuple()):  # easier way to call a piece's move funtion
        if selected:
    ...
    def getAllMoves(self, clr: str):  # get every possible move for one colour
        moves = []
        for i in range(L):
            for j in range(L):
    ...
    def loadColours(self):
        ...
        clrs = OrderedDict({"lightsquare": "white", "darksquare": "gray", "currentsquare": "yellow",
        ...
        try:
            try:  # depending on where the config file is located, load file into memory
                ...
                with open("chess/config.json", 'r') as data:
                    text = json.load(data)
            for key in clrs:
                try:
                    par = text["colours"][key]
                except KeyError:  # if search could not find a valid field
                ...
                try:
                    ...
                    print(f'"{par}" is not a valid colour, please select a colour that pygame will recognise')
                    clrs[key] = pg.Color(clrs[key])
            self.colours = clrs
        except FileNotFoundError:
    ...
    def registerEvent(self, selected, event, screen):
        ...
        if event.type == pg.QUIT:
            ...
            if selected == None:  # if no square is selected
                if self.isOccupied((r, c)) == self.turn:  # if piece on square belongs to the active player
                    selected = (r, c)
                    self.drawScreen(screen, self[r][c].findMoves(False), (r, c))
            else:
                # determines if a move is legal for the selected piece, returns false if not and the move if yes
                move = self.isMove(selected, (r, c))
                if move:  # THIS IS WHERE THE PLAYER MOVES THEIR PIECES
                    ...
                    self.drawScreen(screen)
                    ch = {'b': "BLACK", 'w': "WHITE"}
                    if len(self.allPieces()) <= 2:  # if there are only two or fewer pieces on the board
                        # meaning that, legally, there can only be two kings and therefore no way for either side to win
                        # STALEMATE
                        print("STALEMATE")
                    elif len(self.getAllMoves(self.turn)) == 0:  # if the new active player has no moves
                        if self.checkForCheck(self.turn):  # if the newly active player is in check, meaning that they got checkmated
                            # CHECKMATE
                            print(ch[self.turn] + " HAS BEEN CHECKMATED")
                        else:
                            # STALEMATE
                            print("STALEMATE")
                    elif self.checkForCheck(self.turn):
                        # CHECK
                        print(ch[self.turn] + " HAS BEEN CHECKED")
                        self.lastMove().check = True
                elif (r, c) == selected:  # if you click the already selected square
                    selected = None
                    self.drawScreen(screen)
                elif self.isOccupied((r, c)) == self.turn:  # if piece on square belongs to the active player
                    selected = (r, c)
                    self.drawScreen(screen, self[r][c].findMoves(False), (r, c))
                else:
                    selected = None
            ...
            if event.key == pg.K_BACKSPACE:  # if backspace is pressed, undo last move
                ...
                if selected == None:
                    selected = (7, 7)
                    return
                vectors = {pg.K_UP : (1, 0), pg.K_DOWN : (-1, 0), pg.K_LEFT : (0, -1), pg.K_RIGHT : (0, 1)}
                selected[0] += vectors[event.key][0]
                selected[1] += vectors[event.key][1]    
