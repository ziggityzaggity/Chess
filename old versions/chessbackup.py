...
class Piece:
    ...
    def move(self, move):  # moves a piece: adds move to the moves dictionary, updates the board and itself
        ...
        state[move.end[0]][move.end[1]] = state[self.curpos[0]][self.curpos[1]]
        ...
        self.game.clearMoves()  # cannot redo any further moves, so as to prevent impossible moves
...
class Game:
    ...
    def findMoves(self, selected, test):  # easier way to call a piece's findmoves function
        return self.state[selected[0]][selected[1]].findMoves(test)
    
    def move(self, selected, move):  # easier way to call a piece's move funtion
        self.state[selected[0]][selected[1]].move(move)
        ...
        for i in range(L):
            for j in range(L):  # for every square on the board
                if self.isOccupied((i, j)) == oclr:  # if the square contains a piece of the opposite colour
                    ...
                    for m in list(map(lambda a: a.end, self.state[i][j].findMoves(True))):
        ...
        for i in range(L):
            for j in range(L):
                if self.isOccupied((i, j)) == clr:
                    for move in self.state[i][j].findMoves(False):
                        moves.append(move)
        return moves
    ...
    def undo(self):  # undo the last move
        ...
        if move.special == 'ep':  # en passent
            ...
            state[move.rook.startpos[0]][move.rook.startpos[1]] = state[move.rook.curpos[0]
                                                                        ][move.rook.curpos[1]]  # move the rook to its starting position
        ...
        elif move.special == 'dj':  # doublejump
            state[move.start[0]][move.start[1]].doubleJumped = False  # restore the doubleJumped value
        elif move.special == 'up':  # upgrade
            state[move.start[0]][move.start[1]].type = 'p'
    ...
    def drawPieces(self, screen):
        for i in range(L):
            for j in range(L):
                try:
                    screen.blit(self.images[(self.state[i][j].colour + self.state[i][j].type)], pg.Rect(j*SQ, i*SQ, SQ, SQ))
                except:
                    pass
    ...
    def loadImages(self):
        ...
        for p in ls:
            ...
            whiteP = "w" + p
            try:
                img[blackP] = pg.image.load("images/" + blackP + ".png")
                img[whiteP] = pg.image.load("images/" + whiteP + ".png")
            except:
                img[blackP] = pg.image.load("chess/images/" + blackP + ".png")
                img[whiteP] = pg.image.load("chess/images/" + whiteP + ".png")
        self.images = img
    ...
    def drawBoard(self, screen, *args):
        try:
            ...
            for i in range(L):
                for j in range(L):
                    if (i, j) == p:
                        pg.draw.rect(screen, self.colours["currentsquare"], pg.Rect(j*SQ, i*SQ, SQ, SQ))  # current position of piece, default yellow
                    elif ((i, j) in squares) and ((i + j) % 2 == 1):
                        pg.draw.rect(screen, self.colours["darkpossiblesquare"], pg.Rect(j*SQ, i*SQ, SQ, SQ))  # possible position(s) of piece on dark squares, default seagreen
                    elif ((i, j) in squares) and ((i + j) % 2 == 0):
                        pg.draw.rect(screen, self.colours["lightpossiblesquare"], pg.Rect(j*SQ, i*SQ, SQ, SQ))  # possible position(s) of piece on light sqaures, default lightgreeb
                    elif ((i + j) % 2) == 0:
                        pg.draw.rect(screen, self.colours["lightsquare"], pg.Rect(j*SQ, i*SQ, SQ, SQ))  # light squares, default white
                    else:
                        pg.draw.rect(screen, self.colours["darksquare"], pg.Rect(j*SQ, i*SQ, SQ, SQ))  # dark squares, default gray
        except:
            for i in range(L):
                for j in range(L):
                    if ((i + j) % 2) == 0:
    ...
    def loadColours(self):
        ...
        clrs = {"lightsquare": "white", "darksquare": "gray", "currentsquare": "yellow", "darkpossiblesquare": "seagreen", "lightpossiblesquare": "lightgreen"}
        try:
            ...
            if event.type == pg.QUIT:
                ...
                if selected == None:  # if no square is selected
                    ...
                    if move:  # THIS IS WHERE THE PLAYER MOVES THEIR PIECES
                        ...
                        board.state[selected[0]][selected[1]].move(move)
                        selected = None
                        board.changeTurn()
                        board.drawScreen(screen)
                        ch = {'b': "BLACK ", 'w': "WHITE "}
