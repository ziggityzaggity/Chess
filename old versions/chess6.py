...
class Game:
    ...
    def drawScreen(self, colours, screen, *args):
        screen.fill(pg.Color("white"))
        try:
            drawBoard(colours, screen, list(map(lambda a: a.end, args[0])), args[1])
        except:
            drawBoard(colours, screen)
...
def drawBoard(colours, screen, *args):
    try:
        squares = args[0]
        p = args[1]
        for i in range(L):
            for j in range(L):
                if (i, j) == p:
                    pg.draw.rect(screen, colours["currentsquare"], pg.Rect(j*SQ, i*SQ, SQ, SQ))  # current position of piece
                elif (i, j) in squares:
                    pg.draw.rect(screen, colours["possiblesquare"], pg.Rect(j*SQ, i*SQ, SQ, SQ))  # possible position(s) of piece, default
                elif ((i + j) % 2) == 0:
                    pg.draw.rect(screen, colours["lightsquare"], pg.Rect(j*SQ, i*SQ, SQ, SQ))  # light squares, default white
                else:
                    pg.draw.rect(screen, colours["darksquare"], pg.Rect(j*SQ, i*SQ, SQ, SQ))  # dark squares, default gray
    except:
        for i in range(L):
            for j in range(L):
                if ((i + j) % 2) == 0:
                    pg.draw.rect(screen, colours["lightsquare"], pg.Rect(j*SQ, i*SQ, SQ, SQ))
                else:
                    pg.draw.rect(screen, colours["darksquare"], pg.Rect(j*SQ, i*SQ, SQ, SQ))
    return
    
def loadColours():
    # Default colour values
    clrs = {"lightsquare": "white", "darksquare": "gray", "currentsquare": "yellow", "possiblesquare": "green"}
    try:
        try:
        ...
        return dict([(l, pg.Color(clrs[l])) for l in clrs])
...
def main():
    ...
    colours = loadColours()  # also only called at the beginning, reads config.txt file for colour preferences
    board.drawScreen(colours, screen)
    running = True
    selected = None
    while running:
            ...
            if event.type == pg.QUIT:
                ...
                if selected == None:  # if no square is selected
                    if board.isOccupied((r, c)) == board.turn:  # if piece on square belongs to the active player
                        selected = (r, c)
                        board.drawScreen(colours, screen, board.state[r][c].findMoves(False), (r, c))
                else:
